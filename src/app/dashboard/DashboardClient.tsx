...
export default function DashboardClient({ categories, isAdmin }: Props) {
  ...
  return (
    <div className="space-y-4">
      {/* Tab Bar */}
      <div className="flex items-center gap-1.5 bg-slate-100/80 dark:bg-slate-800/60 backdrop-blur-sm p-1 rounded-2xl w-fit">
        {tabs.map((tab) => {
          const Icon = tab.icon;
          ...
          return (
            <button
              ...
              className={clsx(
                "relative flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-200 select-none",
                active
